...
def _WBA_single(arr):
    N = len(arr)
    weigths = _weights(N + 1)
    return np.sum(weigths * arr, dtype=np.float64)
...
def _WBA_multi_parallel(arr):
    N = len(arr[:, 0])
    weigths = _weights(N)
    return np.sum(weigths * arr[1:, :].T, axis=1, dtype=np.float64)
...
def _WBA_multi_arctan2_parallel(q, p):
    ...
    return np.sum(weigths * phiDiff.T, axis=1, dtype=np.float64)
