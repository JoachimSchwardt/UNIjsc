...
def _2f_signal(freq, freq2, ampl, N, qOffset=0.0, pOffset=0.0, 
               qFactor=None, pFactor=None, rad=1.0):
    """
    ...
    z = (rad * np.exp(2.0*1j*np.pi * freq * Narr) + 
         rad * ampl * np.exp(2.0*1j*np.pi * freq2 * Narr))
...
def freq_along_vector(initial, ax=None, Nq=100, N=1000, K=0.0, 
                      CompareToNaff=0, thresh=1e-5, mapMode='decision',
                      minDiffBoundary=2.0, NaffCont=1, ShowDiff=0, loc='best', 
                      fs=14, tls=12, lfs=10, Show=0, SetTitle=1, SetYlabel=1,
                      RemoveChaos=0):
    [q1, p1, q2, p2] = initial
    q0 = np.linspace(q1, q2, Nq, dtype=np.float64)
    p0 = np.linspace(p1, p2, Nq, dtype=np.float64)
    ...
    if type(ax) == type(None):
        fig, ax = plt.subplots(1 + (CompareToNaff and ShowDiff), 1, 
                               figsize=(16, 9))
    if type(ax) != np.ndarray:
        ax = np.array([ax], dtype=object)
    
    if SetTitle:
        ax[0].set_title(f"Initial conditions from {q1,p1} to {q2,p2}\n" +
                        f"with K={K} and N={N}", fontsize=fs)
    if SetYlabel:
        ax[0].set_ylabel(r"$\nu$", fontsize=fs)
    for i in range(len(ax)):
        if i == (ShowDiff): ax[i].set_xlabel(r"$t$", fontsize=fs)
    ...
    if CompareToNaff:
        ...
        absDiff[indx] = absDiff2[indx]
        if RemoveChaos:
            cInd = np.full(Nq, False, dtype=bool)
            for i in range(Nq):
                Nchaos = 2**11
                q, p = _std_map(q0[i], p0[i], 2*Nchaos, K)
                nu1 = WBA_core._WBA_single(p[:Nchaos])
                nu2 = WBA_core._WBA_single(p[Nchaos:])
                # print(nu1, nu2)
                if np.abs(nu1 - nu2) > 1e-7:
                    cInd[i] = True
            
        ax[0].plot(t[~cInd], freqNaff[~cInd], ls='', marker='o', 
        ...
        if ShowDiff:
            ...
            ax[1].set_yscale('log')
            if SetTitle:
                ax[1].set_title("Frequency difference between Naff and WBA",
                                fontsize=fs)
        
    ax[0].plot(t[~cInd], freqWBA[~cInd], ls='', marker='o', 
               ms=2, mew=1, c='b', label='WBA')
    ax[0].legend(fontsize=lfs, numpoints=3, loc=loc)
    if Show:
        plt.show()
    # return fig, ax
...
def plot_conv(z, freq, c='b'):
    fig, ax = plt.subplots(figsize=(16,9))
    q, p = z.real, z.imag
    Narr = WBA_tools.N_arr(5.0, np.log2(q.shape[0]), 100)
    wba = WBA_core._WBA_arctan2(Narr, q, p)
    naff = WBA_tools._Naff(Narr, q, p, 0)
    abs_diff_wba = np.abs(wba - freq)
    abs_diff_naff = np.abs(naff - freq)
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.plot(Narr, abs_diff_wba, lw=1, c=c)
    ax.plot(Narr, abs_diff_naff, lw=1, ls='--', c=c)
    return wba[-1], naff[-1]

def test_fts():
    from FourierTestSignals import FourierTestSignal as FTS
    fts = FTS([1, 0.2, 0.05, 0.03, 0.01], 
              [np.sqrt(2)-1,0.12345, 0.2853, 0.37496, 0.6180255908734], 
              2**14)
    #fts.add_harmonics(1, [0.05, 0.03, 0.01])
    z = fts.compute_signal()
    freq = fts.freq_array[0]
    return plot_conv(z, freq)
...
if __name__ == "__main__":
    ...
    # art_signal_grid(Nf=100, Nf2=100, ampl=0.5, N=2**12,
    #                 UseNaff=0)
