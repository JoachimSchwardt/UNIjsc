...
try:
    ...
    print("Warning: Module 'mpl_special' not available or not in $PATH!")
...
class LifetimeExtension:
    """Extent the lifetime of a single object by elevating it to a class member
    """
    variable = None

def husimi_dynamisch(event, ax_phase, ax_ort, eigen_werte, eigen_fkt,
                     x_werte, potential, faktor, h_quer, e_max, asym,
                     koeffizienten, husimi_achsen, zeiten, anim_container):
    ...
    if event.button == 1 and event.inaxes == ax_phase and mode == '':
        ...
        wellenpaket = ax_ort.plot(x_werte, np.full_like(x_werte, np.nan),
                                  linewidth=3, color='k')
        # berechne Husimi
        ...
        # Konturlinien und Ticks speichern
        # (zorder funktioniert nicht mit blit=True --> manuell...)
        ...
        def animate(zeit):
            ...
            return wellenpaket[0], husimi_plot, *contour_lines, *xticks, *yticks

        anim_container.variable = matplotlib.animation.FuncAnimation(
            plt.gcf(), animate, frames=zeiten, interval=0, blit=True, repeat=False)
...
def main():
    ...
    anim_container = LifetimeExtension()
