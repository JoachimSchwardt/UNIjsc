...
def husimi_dynamisch(event, ax_phase, ax_ort, eigen_werte, eigen_fkt,
    ...
    if event.button == 1 and event.inaxes == ax_phase and mode == '':
        ...
        for artist in ax_phase.images:
            artist.remove()                              # entferne alle Bilder
        # berechne Husimi
...
def main():
    ...
    fig.canvas.mpl_connect('button_press_event', lambda event: husimi_dynamisch(
        event, ax_phase, ax_ort, eigen_werte, eigen_fkt, x_werte, potential, faktor,
        h_quer, e_max, asym, koeffizienten, husimi_achsen, zeiten)
    )
    plt.show()
