...
inline __m256d _mm256_log_pd(__m256d __A) noexcept {
    __m256d __B;
    #pragma omp simd
    for (int i = 0; i < SIMD_SIZE; ++i) {
        __B[i] = std::log(__A[i]);
...
void normal_distribution(std::array<dtype, N>& rvals,
                         auto uniform, auto uniform_eps, auto gen) noexcept {
    /** Fill a given array with random normals (white noise)

    let u0, u1 be uniform on (0,1].
    then
        z0 = sqrt(-2log(u0)) * sin(2*pi*u1) and
        z1 = sqrt(-2log(u0)) * cos(2*pi*u1)
    are white noise
    */
    __m256d __u_eps, __u, __arg, __sin, __cos, __log, __rn1, __rn2;
    for (int i = 0; i < N; i += 2 * SIMD_SIZE) {
        #pragma omp simd
        for (int lane = 0; lane < SIMD_SIZE; ++lane) {
            __u_eps[lane] = (*uniform_eps)(*gen);
            __u[lane] = (*uniform)(*gen);
        }

        __log = _mm256_mul_pd(_mm256_set1_pd(-2.0), _mm256_log_pd(__u_eps));     // -2*log(u0)
        __log = _mm256_sqrt_pd(__log);                                           // sqrt(...)
        __arg = _mm256_mul_pd(_mm256_set1_pd(2.0 * M_PI), __u);                  // 2*pi*u1
        __sin = _mm256_sin_pd(__arg);                                            // sin(2*pi*u1)
        __cos = _mm256_cos_pd(__arg);                                            // cos(2*pi*u1)
        __rn1 = _mm256_mul_pd(__log, __sin);
        __rn2 = _mm256_mul_pd(__log, __cos);

        #pragma omp simd
        for (int lane = 0; lane < SIMD_SIZE; ++lane) {
            rvals[i + lane] = __rn1[lane];
            rvals[i + lane + SIMD_SIZE] = __rn2[lane];
        }
    }
}
...
//    std::uniform_real_distribution<> normal{-1.0, 1.0};
    constexpr dtype epsilon = std::numeric_limits<dtype>::epsilon();
    std::mt19937 gen(std::random_device{}()); // Standard Mersenne-Twister
    std::uniform_real_distribution<> uniform(0.0, 1.0);
    std::uniform_real_distribution<> uniform_eps(epsilon, 1.0);

    bool State;
    __m256d __temp;
    std::array<dtype, N> x_vals = {0};
    std::array<dtype, N> r_vals = {0};
    for (dtype tval = 0; tval < num_tau * tau; tval += delta_t) {
        State = std::fmod(tval, static_cast<dtype>(tau)) > tau / (1 + theta);
        normal_distribution(r_vals, &uniform, &uniform_eps, &gen);
//        if (tval < delta_t) {
//            for (const auto& elem : r_vals) {
//                std::cout << elem << ' ';
//            }
//        }
...
#pragma GCC ivdep
        for (int i = 0; i < N; i += SIMD_SIZE) {
            ...
            __temp = _mm256_set1_pd(std::sqrt(2.0 * D * delta_t) * r_vals[i]);
