...
//    result[1] = -4.0 * std::pow(y_val[0], 3) + 2.0 * y_val[0] - 0.05;
    result[1] = -4.0 * y_val[0] * y_val[0] * y_val[0] + 2.0 * y_val[0] - 0.05;
    return result;
}
...
//auto adapt_stepsize(const double err, const double tau,
//                    const double atol = 1e-8, const double min_update_ratio = 2.0)
//{
//    /** Adapt the stepsize 'tau' based on the current error estimate. */
//    double abs_err_ratio = err / atol;
//    double tau_new;
//    if (abs_err_ratio < 1.0 / min_update_ratio)
//    {
...
//        tau_new = tau;
//    }
//    else if (abs_err_ratio < min_update_ratio)
//    {
//        tau_new = tau / min_update_ratio;
//    }
//    else
//    {
//        tau_new = tau / (min_update_ratio * std::pow(abs_err_ratio, 0.25));
//    }
//    bool redo_step = (tau_new < tau);
//    return std::make_tuple(tau_new, redo_step);
//}
auto adapt_stepsize(const double err, const double tau, const double atol = 1e-8)
{
    /** Adapt the stepsize 'tau' based on the current error estimate. */
    double abs_err_ratio = err / atol;
    double tau_new;
    if (abs_err_ratio < 0.125)
    {
        tau_new = 2.0 * tau;
    }
    else if (abs_err_ratio < 1.0)
                  ...
                  const double atol)
{
    ...
    double tau = t_final - t_initial;    // try integrating in one step
    ...
    while (t_current < t_final)
    {
        while (true)
        {
            ...
            double err = estimate_error(func, y_current, t_current, y_next, tau);
//            std::tie(tau_new, redo_step) = adapt_stepsize(err, tau, atol);
            auto [tau_new, redo_step] = adapt_stepsize(err, tau, atol);
//            std::cout<<"T_CURRENT: "<<t_current<<", TAU: "<<tau<<", TAU_NEW: "<<tau_new
//            <<", Y_NEXT: "<<y_next[0]<<", "<<y_next[1]<<"\n";
            if (redo_step)
            {
                tau = tau_new;
            }
...
auto rk_int(const func_t func, const vec_t& t_0_values, const vec_t& y_0, const double atol = 1e-8)
{
    ...
    for (size_t i = 1; i < t_0_values.size(); ++i)
    {
        ...
        _rk_int_step(func, t_initial, t_final, y_initial, t_values, y_values, i, atol);
        t_initial = t_final;
    }
    return std::make_pair(t_values, y_values);
}
...
int main()
{
    ...
    Function calls:  4195
    Last point: [-0.03248704, -0.20089087]
    ...
    Last point: [-0.03233259, -0.20079472]

    C++:
    ~3 ms
    Function calls: 28136
    Last point: [-0.250705, 0.421752]   --> how is that posssible? same algorithm...
        - note: result is actually not wrong, the orbit looks very good. Just not consistent with numba code

    change stepsize adaption to only jump factors of 2:
    ~3 ms
    Function calls: 25576
    Last point: [-1.03173, -0.0304257]
    */
...
//    }
    for (size_t i = 0; i < n_values; ++i)
        std::cout << result.first[i] << " :  " << result.second[i][0] << ", " << result.second[i][1] << "\n";
