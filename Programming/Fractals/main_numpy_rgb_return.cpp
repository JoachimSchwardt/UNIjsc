...
using col_array_t = std::vector<std::vector<int>>;
...
__m256i get_counter(const __m256d cxval, const __m256d cyval, const int n_iter) {
...
ctr_array_t get_fractal(const Shape& shape, const Extent& extent_, const int n_iter) {
    ...
    for (int row = 0; row < shape.width; ++row) {
        ...
        for (int col = 0; col < shape.height; col += SIMD_SIZE) {
            ...
            __m256i __counter = get_counter(__xvals, __yvals, n_iter);
...
//auto ctr2rgb(const ctr_array_t& ctr_arr, const col_array_t& col_arr) {
...
//        }
//    }
//    std::cout<<rgb[0][0][0]<<", "<<rgb[0][0][1]<<", "<<rgb[0][0][2]<<"\n";
//    std::cout<<rgb[1][0][0]<<", "<<rgb[1][0][1]<<", "<<rgb[1][0][2]<<"\n";
//    std::cout<<rgb[0][1][0]<<", "<<rgb[0][1][1]<<", "<<rgb[0][1][2]<<"\n";
//
//    return py::array_t<int>({height, width, 3}, &rgb[0][0][0]);    //std::vector<ptrdiff_t>
//}
auto ctr2rgb(const ctr_array_t& ctr_arr, const col_array_t& col_arr) {
    ...
    */
    ...
    auto rgb = py::array_t<int>(height * width * 3);
    rgb.resize({height, width, 3});
    auto rgb_arr = rgb.mutable_unchecked<3>();

    #pragma omp parallel for
    for (int row = 0; row < height; ++row) {
        for (int col = 0; col < width; ++col) {
            for (int j = 0; j < 3; ++j) {
                rgb_arr(row, col, j) = col_arr[ctr_arr[row][col]][j];
            }
        }
    }
    return rgb;
...
$(/home/joachim/.conda/envs/spyder/bin/python3.9 -m pybind11 --includes) main_numpy_rgb_return.cpp \
-lm -lmvec -o fractal_cpp$(python3.9-config --extension-suffix --ldflags)
*/
