...
//    Extent() : Extent(-0.25, 0.25, -0.25, 0.25) { }
    Extent() : Extent(-2.0, 2.0, -2.0, 2.0) { }
    dtype xmin;
    dtype xmax;
    dtype ymin;
    dtype ymax;
...
template <typename F>
ctr_array_t get_fractal(F get_counter, const Shape& shape, const Extent& extent_,
                        const std::vector<dtype>& z_0, const int n_iter) {
    ...
    for (int row = 0; row < shape.width; ++row) {
        ...
        for (int col = 0; col < shape.height; col += SIMD_SIZE) {
            ...
            _mm256_storeu_si256((__m256i*)&ctr_array[row][col], __counter);
        }
    }
...
rgb_array_t ctr2rgb(const ctr_array_t& ctr_arr, const col_array_t& col_arr) {
    /** https://www.linyuanshi.me/post/pybind11-array/
    */
    int height = ctr_arr.size();
    int width = ctr_arr[0].size();
    rgb_array_t rgb(height, col_array_t(width));

////    #pragma omp parallel for
//    for (int row = 0; row < height; ++row) {
//        for (int col = 0; col < width; ++col) {
//            rgb[row][col] = col_arr[ctr_arr[row][col]];
//        }
//    }
    for (int row = 0; row < height; ++row) {
        for (int col = 0; col < width; col += 8) {
            __m256i indx = _mm256_loadu_si256((__m256i*)&ctr_arr[row][col]);           // SIMD version
            __m256i rgba_val = _mm256_i32gather_epi32((int*)&col_arr[0], indx, 4);     // 4 Byte (uint32)
            _mm256_storeu_si256((__m256i*)&rgb[row][col], rgba_val);
        }
    }
    return rgb;
}
...
int main() {
    ...
    10000,
        ...
        :: 0.34 sec (SAME THING, but height/width swap
                      --> where do we lose 10% performance?
                      --> test within region where all orbits stay bounded
                      --> no performance loss, just "unfortunate" choice of extent/resolution!)

    10000,  1024,  1024,   (-0.25, 0.25, -0.25, 0.25)
        :: 0.36 sec (this is the same for the other version)
    ...
    */
    ...
    const std::vector<dtype> z_0 = {0.0, 0.0};

    col_array_t col_arr(n_iter + 1);

    int ctr = 0;
    for (uint32_t rgbint = 0; rgbint < 273948778; rgbint += 124) {
        col_arr[ctr] = rgbint;
        if (ctr > n_iter) break;
        ++ctr;
    }
...
//    }
    ...
    ctr_array_t ctr_arr = get_fractal(get_counter_mandelbrot, shape, extent_, z_0, n_iter);
//    ctr_array_t ctr_arr = get_fractal(get_counter_julia, shape, extent_, z_0, n_iter);
...
/**PYBIND11_MODULE(fractal_cpp, m) {
    ...
    m.def("get_fractal", &get_fractal, "Return a 2d-array of integers corresponding to the final iteration for each pixel",
        py::arg("shape"), py::arg("extent"), py::arg("n_iter"));
    m.def("ctr2rgb", &ctr2rgb, "Select RGB values from an array depending on the respective 'ctr'-value",
        py::arg("ctr_arr"), py::arg("col_arr"));
} */

/** c++ -Wall -O3 -march=native -mavx2 -ffast-math -fopenmp -shared -std=c++2a -fPIC \
$(/home/jo-cube/miniconda3/envs/spyder/bin/python3.11 -m pybind11 --includes) main_rgb_transposed_pybind.cpp \
