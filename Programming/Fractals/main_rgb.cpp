...
ctr_array_t get_fractal(const Shape& shape, const Extent& extent_, const int n_iter) {
    ...
    ctr_array_t ctr_array(shape.height, std::vector<long>(shape.width));

    #pragma omp parallel for
    for (int row = 0; row < shape.height; ++row) {
        __m256d __yvals = _mm256_set1_pd(yvals[row]);            // all yvals are the same
        for (int col = 0; col < shape.width; col += SIMD_SIZE) {
            ...
            __m256d __xvals = _mm256_loadu_pd(&xvals[col]);      // load 4 x-values in the same row
...
//    std::cout<<elem[0]<<", "<<elem[1]<<", "<<elem[2]<<'\n';}
    ...
    rgb_array_t rgb = ctr2rgb(ctr_arr, col_arr);
...
-o fractal_cpp$(python3.9-config --extension-suffix)
*/
